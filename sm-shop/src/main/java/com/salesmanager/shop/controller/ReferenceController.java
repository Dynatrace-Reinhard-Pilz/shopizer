...
//   }	
	...
	// @RequestMapping(value="/shop/reference/zoneName")
	// public @ResponseBody String zoneName(@RequestParam String zoneCode, HttpServletRequest request, HttpServletResponse response) {
	// 	Span span = getTracer().spanBuilder("SELECT dual.users")
	// 	.setSpanKind(SpanKind.CLIENT)
	// 	.setAttribute("db.statement", "SELECT user FROM dual")
	// 	.setAttribute("db.name", "dual")
	// 	.setAttribute("db.system", "mssql")
	// 	.setAttribute("db.user", "system")
	// 	.setAttribute("db.operation", "SELECT")
	// 	.setAttribute("db.sql.table", "dual")
	// 	.setAttribute("db.connection_string", "Server=(localdb)\\v11.0;Integrated Security=true;")
	// 	.startSpan();
    //     try (Scope scope = span.makeCurrent()) {
	// 		// try {
	// 		// 	java.net.URL url = new java.net.URL("http://localhost:8090/hello");
    ...
    //         // span.setAttribute("newOrderId", "1111");
	// 	try {
	// 		Language language = languageUtils.getRequestLanguage(request, response);
	// 		if(language==null) {
	// 			return zoneCode;
