...
public class ShoppingCategoryController {
	...
	private void displayCategory() {
		Span span = getTracer().spanBuilder("display-category").startSpan();
	...
	private void validateCategory() {
		Span span = getTracer().spanBuilder("validate-category").startSpan();
		try (Scope scope = span.makeCurrent()) {
			recheckCategory();
		} finally {
			span.end();
	...
	private void loadCategory() {
		Span span = getTracer().spanBuilder("query-category").startSpan();
		try (Scope scope = span.makeCurrent()) {			
		} finally {
			span.end();
		}
	...
	private void persistCategory() {
		Span span = getTracer().spanBuilder("persist-category").startSpan();
		try (Scope scope = span.makeCurrent()) {
		} finally {
			span.end();
		}
				...
				if(manufacturerList==null) {
					...
					//if(missedContent==null) {
						...
						if(manufacturerList.isEmpty()) {
							cache.putInCache(Boolean.TRUE, manufacturersKeyMissed.toString());
						} else {
