...
public class LocalImageFilePathUtils extends AbstractimageFilePath{
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(LocalImageFilePathUtils.class);
	...
	public String buildStoreLogoFilePath(MerchantStore store) {
		// LOGGER.info("getBasePath(store): " + getBasePath(store));
	...
	private String getScheme(MerchantStore store) {
		String res = store.getDomainName();
		res = res.replace("http://localhost:8080", "");
		// if (res.contains("localhost")) {
		// 	int idx = Constants.DEFAULT_DOMAIN_NAME.indexOf(":");
		// 	if (idx > 0) {
		// 		String repl = Constants.DEFAULT_DOMAIN_NAME.substring(0, idx);
		// 		res = res.replace("localhost", repl);
		// 	}
		// }
		return res;
	}
