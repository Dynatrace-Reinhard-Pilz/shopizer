			...
			if (breadCrumb == null) {
				...
				if (language.getCode().equals(breadCrumb.getLanguage().getCode())) {
					...
					for (BreadcrumbItem item : breadCrumb.getBreadCrumbs()) {
						...
						if (item.getItemType().name().equals(BreadcrumbItemType.HOME.name())) {
							...
							items.add(homeItem);
						} else if (item.getItemType().name().equals(BreadcrumbItemType.PRODUCT.name())) {
							Product product = null;
							try {
								product = productService.getProductForLocale(item.getId(), language, locale);
						...
						} else if (item.getItemType().name().equals(BreadcrumbItemType.CATEGORY.name())) {
							Category category = null;
							if ((categoryService != null) && (item != null)) {
								try {
									category = categoryService.getOneByLanguage(item.getId(), language);
								} catch (Throwable thrown) {
						...
						} else if (item.getItemType().name().equals(BreadcrumbItemType.PAGE.name())) {
