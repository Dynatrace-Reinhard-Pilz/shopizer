...
public class PriceByDistanceShippingQuoteRules implements ShippingQuoteModule {
	...
	public static final Logger LOGGER = LoggerFactory.getLogger(PriceByDistanceShippingQuoteRules.class);
	...
	public List<ShippingOption> getShippingQuotes(ShippingQuote quote,
		...
		//maximum distance TTODO configure from admin
		if(distance > 150D) {
			return null;
		}
		...
		if(distance<=20) {
			price = new BigDecimal(2);//TTODO from the admin
		} else {
			price = new BigDecimal(3);//TTODO from the admin
		}
